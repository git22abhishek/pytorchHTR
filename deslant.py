# From: https://github.com/githubharald/DeslantImg Copyright (c) 2018 Harald Scheidl
...
def shear_img(img: np.ndarray, s: float, bg_color: int, interpolation=cv2.INTER_NEAREST) -> np.ndarray:
    ...
    img_sheared = cv2.warpAffine(
        img, shear_transform, (w, h), flags=interpolation, borderValue=bg_color)
...
def deslant(img: np.ndarray,
    ...
    img_mask = cv2.threshold(
        255 - img, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1] // 255
    ...
    res_shear_val = sorted(candidates, key=lambda c: c.score, reverse=True)[
        0].shear_val
